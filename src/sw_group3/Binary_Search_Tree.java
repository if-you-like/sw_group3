...
public class Binary_Search_Tree extends NodeStructure {
	...
	void ArrayInput(Node t, int n, int tmp) {
		if (t != null) {
			ArrayInput(t.left, n - tmp, tmp / 2);
			TreeData[n] = t.data;
			ArrayInput(t.right, n + tmp, tmp / 2);
		}
	}
	...
	int TreeDepth(Node temp) {
		if (temp != null) {
			return 1 + Math.max(TreeDepth(temp.left), TreeDepth(temp.right));
		} else {
			return 0;
		}
	}
